...
export default function MessageList({
  ...
  return (
    ...
    >
      {isFetchingNextPage && (
        <div className="py-2 text-center text-caption text-text-secondary">
          이전 메시지를 불러오는 중...
        </div>
      )}
      ...
      <ul className="flex flex-col gap-3">
        {ordered.map((message, index) => {
          ...
          const showDivider =
            prev === null || !isSameDay(prev.createdAt, message.createdAt);

          return (
            <li key={message.id} className="flex flex-col gap-3">
              {showDivider && (
                <div className="flex justify-center py-1">
                  <span className="rounded-full bg-bg-primary-darker px-3 py-1 text-caption font-regular text-text-secondary">
